...
pub fn recv_loop(config: &ServiceConfig) -> Result<(), failure::Error> {
    ...
    let slave = config
        ...
        .unwrap_or(0x00) as u16;
    
    let path = config
        .get("path")
        .and_then(|i| i.as_str().map(|str| str.to_owned()))
        .unwrap_or("/dev/i2c_0".to_string());
    ...
    let c_protocol = cbor_protocol::Protocol::new(I2CStream::new(&path, slave), transfer_chunk_size);
            ...
            .contains_key(&channel_id)
        {
            ...
            thread::spawn(move || {
                ...
                let f_protocol = FileProtocol::new(
                    I2CStream::new(&path_ref,slave_ref),
                    config_ref,
                );
